...
type ManagerEvents struct {
	...
	// Fired when an unknown peer connects
	ConnectedUnknown *events.Event
...
// PeerConnUnknownIncomingCaller gets called with a peed.ID and its associated network.Conn.
func PeerConnUnknownIncomingCaller(handler interface{}, params ...interface{}) {
	handler.(func(peer.ID, network.Conn))(params[0].(peer.ID), params[1].(network.Conn))
}
...
func NewManager(host host.Host, opts ...ManagerOption) *Manager {
	...
	mng := &Manager{
		Events: ManagerEvents{
			...
			ConnectedUnknown:   events.NewEvent(PeerConnUnknownIncomingCaller),
...
func (m *Manager) eventLoop(shutdownSignal <-chan struct{}) {
	for {
		...
		case connectedMsg := <-m.connectedChan:
			...
			if p == nil {
				m.Events.ConnectedUnknown.Trigger(connectedMsg.conn.RemotePeer(), connectedMsg.conn)
			} else {
...
func (m *Manager) registerLoggerOnEvents() {
	...
	m.Events.ConnectedUnknown.Attach(events.NewClosure(func(id peer.ID, conn network.Conn) {
		m.opts.Logger.Infof("connection from unknown peer %s (%s)", id.ShortString(), conn.Stat().Direction.String())
	}))
